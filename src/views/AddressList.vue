<template>
  <div>
    ...
    <Modal :mdShow="isMdShow" @closeModal="closeModal">
      <p slot="message">你是否确认要删除此地址</p>
      <div slot="btnGroup">
        <a href="javasceipt:;" class="btn btn--m" @click="delAddressAct">确认</a>
        <a href="javscript:;" class="btn btn--m" @click="closeModal">取消</a>
      </div>
    </Modal>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      getaddress() {
        this.axios.post(this.baseUrl+ '/users/addressList',qs.stringify({
          userId: 100000077
        })).then(response => {
      ...
      setDefault(id) {
        this.axios.post(this.baseUrl + '/users/setDefault', qs.stringify({
          userId: 100000077,
          addressId: id
        })).then(response => {
      ...
      delAddressAct() {
        this.axios.post(this.baseUrl + '/users/delAddress',qs.stringify({
          userId: 100000077,
