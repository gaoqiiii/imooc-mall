<template>
  <div id="cart">
        ...
        <div class="item-list-wrap">
          <div class="cart-item">
            ...
            <ul class="cart-item-list">
              <li v-for="(item,index) in cartList" :key="index" v-if="cartList">
    ...
    <Modal :mdShow="checkDel" v-on:closeModal="closeCheckDel">
      <p slot="message">
        你确认要删除此条数据吗？
      </p>
      <div slot="btnGroup">
        <a href="javascript:;" class="btn btn--m" @click="closeCheckDel">关闭</a>
...
<script>
  ...
  export default {
    ...
    methods: {
      init() {
        this.axios.post(this.baseUrl + '/users/cartList',qs.stringify({
          userId: 100000077
        })).then(res => {
      ...
      editCart(flag,item) {
        ...
        this.axios.post(this.baseUrl + '/users/cartEdit',qs.stringify({
          userId: 100000077,
      ...
      toggleCheckAll() {
        ...
        this.axios.post(this.baseUrl+ '/users/editCheckAll', qs.stringify({
          userId: 100000077,
          checkAll: flag
        })).then(response => {
          let res = response.data
